...
- [x] **Interview prep mode** — from a saved tracker card, generate a Gemini-backed set of likely behavioral and technical questions tailored to the JD and the user's stored profile, with suggested answer structures pre-filled from memory. Keeps interview prep local-first and zero-upload.
  - Fix (2026-09-17): two latent bugs made this effectively non-functional for tracked jobs. (1) Opening interview prep for *any* already-tracked application sent a `GET_APPLICATION` message with no registered background handler (`background/message-router.js` has no such entry), so it always failed. (2) When the active browser tab had no detectable job posting, the screen showed "No job detected" even when the pipeline had active jobs, instead of falling back to them. Fixed by reading applications from the existing `GET_STATE` payload (same source Pipeline uses — no second source of truth) and falling back to the most recently updated active pipeline job when no page job is detected. Added a compact job-readiness bubble bar (`#interview-prep-job-bar`) so users can switch which tracked job they're prepping for. Covered by `tests/interview-prep-job-detection.test.mjs`.
